...
impl GeminiHookResponse {
    ...
    /// Whether this response blocks the operation, in any of the forms Gemini
    /// accepts: an explicit deny/block decision, a hard stop (`continue: false`),
    /// or a tool-selection config that disables every tool (`mode: NONE`).
    pub fn is_deny(&self) -> bool {
        let decision_blocks = matches!(self.decision, Some(Decision::Deny | Decision::Block));
        let stops = self.continue_execution == Some(false);
        let tools_disabled = self
            .hook_specific_output
            .as_ref()
            .and_then(|output| output.tool_config.as_ref())
            .and_then(|config| config.mode.as_deref())
            == Some("NONE");
        decision_blocks || stops || tools_disabled
    }
