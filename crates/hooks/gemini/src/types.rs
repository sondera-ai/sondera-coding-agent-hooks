...
/// BeforeModel hook event data.
#[derive(Debug, Deserialize, Serialize)]
pub struct BeforeModelEvent {
    #[serde(flatten)]
    pub common: CommonInput,
    #[serde(default)]
...
/// AfterModel hook event data. Streaming responses may contain a single chunk.
#[derive(Debug, Deserialize, Serialize)]
pub struct AfterModelEvent {
    #[serde(flatten)]
    pub common: CommonInput,
    #[serde(default)]
    pub llm_request: Value,
...
impl BeforeModelEvent {
    pub fn validate(&self) -> Result<()> {
        Ok(())
    }
}

impl AfterModelEvent {
    pub fn validate(&self) -> Result<()> {
        Ok(())
    }
}
