...
pub enum NotificationType {
    ...
    ElicitationDialog,
    /// An MCP elicitation form was submitted or dismissed.
    ElicitationComplete,
    /// An MCP elicitation response was sent to the server.
    ElicitationResponse,
    /// A background session needs user input.
    AgentNeedsInput,
    /// A background session finished or failed.
    AgentCompleted,
    #[default]
    #[serde(other)]
    Unknown,
...
pub enum SessionEndReason {
    ...
    /// User logged out
    Logout,
    /// User exited while prompt input was visible. Claude can resume the same
    /// session later, so this is not a terminal completion signal.
    PromptInputExit,
    /// Bypass permissions mode was disabled
    BypassPermissionsDisabled,
    /// Other exit reasons. Unknown future reasons are treated as resumable by
    /// default to avoid falsely completing long-lived sessions.
    #[default]
    #[serde(other)]
    Other,
}
...
pub struct StopEvent {
    ...
    /// The assistant's final message text (provided by Claude Code).
    #[serde(default)]
...
pub struct SessionStartEvent {
    ...
    /// Model identifier (e.g. "claude-sonnet-4-20250514"). Affects capability profile.
    #[serde(default)]
...
/// Type of configuration change that triggered the ConfigChange event.
#[derive(Debug, Clone, Copy, Deserialize, Serialize, Default, PartialEq)]
#[serde(rename_all = "snake_case")]
...
/// Config change event data. Fires when Claude Code settings change mid-session.
/// Matcher values correspond to which settings layer changed.
#[derive(Debug, Deserialize, Serialize)]
pub struct ConfigChangeEvent {
    ...
    pub config_type: ConfigChangeType,
    /// Optional path associated with file-backed configuration changes.
    #[serde(default)]
    pub file_path: String,
}

/// Instructions loaded event data. Fires when CLAUDE.md or .claude/rules/*.md files are loaded.
#[derive(Debug, Deserialize, Serialize)]
pub struct InstructionsLoadedEvent {
    pub session_id: String,
    pub transcript_path: String,
    #[serde(default)]
    pub cwd: String,
    ...
    #[serde(default)]
    pub file_path: String,
    /// Type of memory/instruction (e.g. "project", "user").
    #[serde(default)]
    pub memory_type: String,
    /// Reason the instructions were loaded (e.g. "initial_load", "file_changed").
    #[serde(default)]
    pub load_reason: String,
}

/// Worktree remove event data. Fires when a git worktree is removed.
#[derive(Debug, Deserialize, Serialize)]
pub struct WorktreeRemoveEvent {
    pub session_id: String,
    pub transcript_path: String,
    #[serde(default)]
    pub cwd: String,
...
mod tests {
    ...
    #[test]
    fn test_config_change_accepts_current_source_field() {
        let json = r#"{
            "session_id": "abc123",
            "transcript_path": "/tmp/transcript.jsonl",
            "hook_event_name": "ConfigChange",
            "source": "policy_settings",
            "file_path": "/etc/managed/claude.json"
        }"#;

        let event: ConfigChangeEvent = serde_json::from_str(json).unwrap();
        assert_eq!(event.config_type, ConfigChangeType::PolicySettings);
        assert_eq!(event.file_path, "/etc/managed/claude.json");
    }
