...
fn parse_tool_output_payload(output: &serde_json::Value) -> serde_json::Value {
    output
        .as_str()
...
/// Handle workspaceOpen hook.
pub fn handle_workspace_open(event: WorkspaceOpenEvent) -> HookResponse {
    info!(
        workspace_roots = ?event.common.workspace_roots,
        "Cursor workspace opened"
    );
    HookResponse::workspace_open_ok()
}
...
impl<H: HarnessClient> Hooks<H> {
    ...
    pub async fn handle_before_shell_execution(
        ...
        let ev = self.event(&event.common.conversation_id, trajectory_event);
    ...
    pub async fn handle_after_mcp_execution(
        ...
        let ev = self.event(&event.common.conversation_id, tool_output);
    ...
    pub async fn handle_after_file_edit(
        ...
        let observation = TrajectoryEvent::Observation(Observation::FileOperationResult(
            FileOperationResult::success(&event.common.generation_id)
                .with_path(&event.file_path)
    ...
    pub async fn handle_after_agent_response(
        ...
        let ev = self.event(&event.common.conversation_id, response);
    ...
    pub async fn handle_subagent_start(
        ...
        // Spawning a subagent is an action the harness can gate, so adjudicate
        // it. `?` propagates a harness failure into `resolve_hook`'s fail-closed
        // `subagent_start_deny`; anything short of an explicit Allow denies.
        let call_id = if event.tool_call_id.is_empty() {
            event.common.generation_id.clone()
        } else {
        ...
        let response = match adjudicated.decision {
            Decision::Allow => {
                info!("Subagent {:?} start allowed", event.subagent_type);
                HookResponse::subagent_start_allow()
            }
            Decision::Deny => {
                let msg = adjudicated.deny_message("Subagent start denied by policy");
                warn!("Subagent start denied: {}", msg);
                HookResponse::subagent_start_deny(msg)
            }
            Decision::Escalate => {
                // subagentStart has no approval channel; escalation cannot be
                // surfaced to the user, so deny rather than silently allow.
                let msg = adjudicated.deny_message("Subagent start requires approval");
                warn!(
                    "Subagent start escalated; denying (no approval channel): {}",
                    msg
                );
                HookResponse::subagent_start_deny(msg)
            }
    ...
    pub async fn handle_subagent_stop(&mut self, event: SubagentStopEvent) -> Result<HookResponse> {
        info!(
            ...
            event.subagent_type, event.status, event.duration_ms
        );
    ...
    pub async fn handle_post_tool_use(&mut self, event: PostToolUseEvent) -> Result<HookResponse> {
        let payload = parse_tool_output_payload(&event.tool_output);
        let observation = match event.tool_name.as_str() {
            ...
            "Read" | "Write" | "Edit" => {
                let content = payload
                    .get("content")
                    .or_else(|| payload.get("file").and_then(|f| f.get("content")))
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string());
            ...
            "WebFetch" => {
                let url = event
                    ...
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                let code = payload.get("code").and_then(|v| v.as_i64()).unwrap_or(200) as i32;
                let result = payload
                    .get("result")
            ...
            _ => Observation::ToolOutput(ToolOutput::success(&event.tool_use_id, payload)),
        };
