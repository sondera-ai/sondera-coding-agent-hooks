...
/// Selected model parameter for hook invocations.
#[derive(Debug, Clone, Deserialize, Serialize, Default)]
pub struct ModelParam {
    /// Parameter identifier, such as `thinking`, `context`, or `effort`.
    #[serde(default)]
    pub id: String,
    /// Parameter value as reported by Cursor.
    #[serde(default)]
    pub value: String,
}

/// Common input fields present in agent and tab hook events.
#[derive(Debug, Clone, Deserialize, Serialize, Default)]
pub struct CommonInput {
    ...
    /// Legacy model slug configured for the composer that triggered the hook.
    #[serde(default)]
    pub model: String,
    /// Structured ID for the selected model, when available.
    #[serde(default)]
    pub model_id: Option<String>,
    /// Selected model parameters, such as thinking, context, or effort.
    #[serde(default)]
...
pub struct BeforeShellExecutionEvent {
    ...
    #[serde(default)]
    pub cwd: String,
    /// Whether the command is running in a sandboxed environment.
    #[serde(default)]
    pub sandbox: bool,
    /// Timeout in milliseconds for the shell command, if provided by older clients.
    #[serde(default)]
...
pub struct SubagentStopEvent {
    ...
    /// Number of follow-up loops already triggered for this subagent.
    #[serde(default)]
    pub loop_count: u32,
...
// ============================================================================
// App lifecycle hook events
// ============================================================================

/// workspaceOpen hook event data.
#[derive(Debug, Deserialize, Serialize)]
...
impl BeforeShellExecutionEvent {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.command.trim().is_empty() {
            return Err(HookError::message("command cannot be empty"));
        }
        Ok(())
    }
}
...
impl BeforeReadFileEvent {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.file_path.trim().is_empty() {
            return Err(HookError::message("file_path cannot be empty"));
        }
        Ok(())
    }
}
