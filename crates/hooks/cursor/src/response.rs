...
// reason: Cursor response-builder API surface is exposed for hook authors; several
// constructors aren't yet exercised by in-tree consumers. Dead-code analysis treats
// pub items in this crate's public API as reachable, so the lint doesn't fire here
// anyway — but keeping the allow makes the intent explicit if the visibility ever changes.
#![allow(dead_code)]
...
pub struct SubagentStartResponse {
    /// Permission decision.
    pub permission: PermissionDecision,
    /// Message shown to the user when denied.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub user_message: Option<String>,
}
...
pub struct PreToolUseResponse {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub user_message: Option<String>,
    /// Message sent to the agent when denied.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub agent_message: Option<String>,
...
impl PreToolUseResponse {
    ...
    pub fn deny(user_message: impl Into<String>) -> Self {
        ...
        Self {
            permission: PermissionDecision::Deny,
            agent_message: Some(user_message.clone()),
...
/// Response structure for workspaceOpen hooks.
#[must_use]
#[derive(Debug, Default, Serialize, Deserialize)]
pub struct WorkspaceOpenResponse {
    /// Absolute plugin directories to load for the current workspace.
    #[serde(rename = "pluginPaths", skip_serializing_if = "Option::is_none")]
    pub plugin_paths: Option<Vec<String>>,
}

impl WorkspaceOpenResponse {
    /// Create a response that does not register additional plugins.
    pub fn ok() -> Self {
        Self::default()
    }
}
...
pub enum HookResponse {
    ...
    Stop(StopResponse),
    /// Response for workspaceOpen hooks
    WorkspaceOpen(WorkspaceOpenResponse),
...
impl HookResponse {
    ...
    /// Create a response for workspaceOpen with no extra plugins.
    pub fn workspace_open_ok() -> Self {
        Self::WorkspaceOpen(WorkspaceOpenResponse::ok())
    }
...
impl HookResponse {
    ...
    pub fn is_deny(&self) -> bool {
        match self {
            ...
            Self::PreToolUse(r) => r.permission == PermissionDecision::Deny,
            Self::SessionStart(r) => !r.continue_execution,
            Self::SubagentStart(r) => r.permission == PermissionDecision::Deny,
            Self::Stop(_) | Self::PreCompact(_) | Self::WorkspaceOpen(_) | Self::NoOutput(_) => {
                false
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_session_start_block_is_deny() {
        let response = HookResponse::session_start_block("Blocked by policy");
