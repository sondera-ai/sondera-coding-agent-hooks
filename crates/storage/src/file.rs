...
/// Append an event to its trajectory's JSONL file.
///
/// The append does `create_dir_all` + `open` + `write`, all synchronous. Run
/// inline from the adjudication path it would block a tokio worker for the
/// duration of the I/O, stalling other in-flight adjudications against their
/// deadlines, so it runs on the blocking pool instead.
///
/// # Errors
///
/// Returns an error if the write fails or the blocking task panics.
pub async fn write_trajectory_event(event: &Event) -> Result<()> {
    ...
    .await
    .context("Trajectory event writer panicked")?
}
