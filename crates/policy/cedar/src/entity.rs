...
///
/// ```
/// # use sondera_cedar_policy::EntityBuilder;
...
/// The namespace the base schema declares (`namespace Sondera { … }`).
///
/// Cedar type names are namespace-qualified, so an `Agent::"x"` built against a
/// `Sondera`-namespaced schema is a *different, undeclared* type and
/// `Request::new` rejects it before any policy runs. Qualifying centrally in
/// [`euid`] keeps every call site writing the bare name.
pub const SCHEMA_NAMESPACE: &str = "Sondera";

/// Construct an `EntityUid` from a bare type name and an ID string.
///
/// The type name is always qualified into `SCHEMA_NAMESPACE`. There is no
/// escape hatch for a pre-qualified or foreign-namespace name: the engine serves
/// exactly one schema, so admitting a second spelling of the same type would buy
/// nothing but ambiguity. A caller that passes `"Sondera::Agent"` anyway gets the
/// nested `Sondera::Sondera::Agent` — a well-formed but undeclared type, which
/// `Request::new` rejects against the schema rather than silently accepting.
pub fn euid(type_name: &str, id: &str) -> Result<EntityUid> {
    let entity_type: EntityTypeName = format!("{SCHEMA_NAMESPACE}::{type_name}")
        .parse()
...
mod tests {
    ...
    fn build_entity_no_attrs() {
        ...
        // Bare names are qualified into the base schema's namespace.
        assert_eq!(entity.uid().type_name().to_string(), "Sondera::Agent");
    }

    /// There is no pre-qualified escape hatch. Nested namespaces are legal
    /// Cedar, so this parses — into a type the schema does not declare, which
    /// `Request::new` then rejects. The point is that it is never silently
    /// treated as a second spelling of `Sondera::Agent`.
    #[test]
    fn euid_double_qualifies_an_already_qualified_type() {
        let uid = euid("Sondera::Agent", "a1").unwrap();
        assert_eq!(uid.type_name().to_string(), "Sondera::Sondera::Agent");
    }
