...
//!   ollama serve
//!
//! Run with: cargo test -p sondera-cedar-policy -- --ignored trajectory_label

use sondera_cedar_policy::{CedarPolicyHarness, Label, Settings, Trajectory, euid};
use sondera_types::{
    Action, Actor, Agent, Control, Decision, Event, Harness, Observation, Prompt, Started,
    TrajectoryEvent, WebFetch,
};
...
async fn load_harness() -> (CedarPolicyHarness, tempfile::TempDir) {
    ...
    let harness =
        CedarPolicyHarness::from_config_dir_isolated_with_settings(path, temp_dir.path(), settings)
            .await
...
async fn start_trajectory(harness: &CedarPolicyHarness, trajectory_id: &str) {
    let started = TrajectoryEvent::Control(Control::Started(Started::new(test_agent())));
    let event = Event::new(test_agent(), trajectory_id, started);
...
async fn get_trajectory(harness: &CedarPolicyHarness, trajectory_id: &str) -> Trajectory {
...
async fn trajectory_entity_store_roundtrip_preserves_label() {
    ...
    harness.upsert_entity(entity).await.unwrap();

    // Read it back and verify the label survived.
    let readback = get_trajectory(&harness, &trajectory_id).await;
    assert_eq!(
        readback.label,
...
async fn trajectory_label_raised_to_highly_confidential_by_pii_prompt() {
    ...
    let traj = get_trajectory(&harness, &trajectory_id).await;
    assert_eq!(
        traj.label,
        Label::Public,
    ...
    let event =
        Event::new(test_agent(), &trajectory_id, prompt).with_actor(Actor::human("test-agent"));
    let result = harness.adjudicate(event).await.unwrap();
    assert_eq!(
        result.decision,
        Decision::Allow,
...
async fn trajectory_label_persists_across_adjudications() {
    ...
    let event =
        Event::new(test_agent(), &trajectory_id, prompt).with_actor(Actor::human("test-agent"));
    let _ = harness.adjudicate(event).await.unwrap();
    ...
    let event = Event::new(test_agent(), &trajectory_id, web_fetch);
    let result = harness.adjudicate(event).await.unwrap();
    ...
    // 6. Verify the denying policy is identified in the adjudication metadata.
