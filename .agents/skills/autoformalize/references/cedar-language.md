...
Load when you need Cedar syntax or semantics itself — operator behaviour, schema
declaration forms, templates, or how the evaluator resolves a decision. For the
Sondera-specific actions, entities, and context fields a policy matches on, load
`sondera-context.md` instead.
